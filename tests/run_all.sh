...
for t in RoNet.luau core/_smoke.luau tests/_smoke_layers.luau tests/_smoke_optim.luau tests/_smoke_train.luau tests/_smoke_bpe.luau tests/_smoke_lm.luau tests/_smoke_parallel.luau tests/_smoke_pt.luau; do
	if [ ! -f "$t" ]; then
		continue
	fi
	printf '== %s ==\n' "$t"
